...
var _audio_context = __webpack_require__(1);
...
var _mic = __webpack_require__(11);

var _mic2 = _interopRequireDefault(_mic);

var _file_audio = __webpack_require__(9);

var _file_audio2 = _interopRequireDefault(_file_audio);

var _biquad_filter = __webpack_require__(5);

var _biquad_filter2 = _interopRequireDefault(_biquad_filter);

var _convolver = __webpack_require__(6);
...
var _dynamics_compressor = __webpack_require__(8);

var _dynamics_compressor2 = _interopRequireDefault(_dynamics_compressor);

var _gain = __webpack_require__(10);

var _gain2 = _interopRequireDefault(_gain);

var _stereo_panner = __webpack_require__(14);

var _stereo_panner2 = _interopRequireDefault(_stereo_panner);

var _wave_shaper = __webpack_require__(15);

var _wave_shaper2 = _interopRequireDefault(_wave_shaper);

var _periodicwave = __webpack_require__(13);
...
var AudioLine = function () {
	function AudioLine(wrapper) {
		_classCallCheck(this, AudioLine);

		var _this = this;
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		_this.update();
	}

	_createClass(AudioLine, [{
		key: 'clear',
		value: function clear() {
			var _this = this;
			_jquery2.default.each(_this.nodeMap, function (i, node) {
				if (node.disconnect) {
					node.disconnect();
				}
			});
		...
		value: function update() {
			var _this = this;
			var $srcDOM = (0, _jquery2.default)('div[data-srctype]', _this.$wrapper);
			var srcType = $srcDOM.data('srctype');
			_this.clear();
			if (!$srcDOM.data('inited')) {
				switch (srcType) {
					case 'oscillator':
						_this.srcWidget = new _oscillator2.default($srcDOM);
						break;
					case 'mic':
						_this.srcWidget = new _mic2.default($srcDOM);
						break;
					case 'audio':
						_this.srcWidget = new _file_audio2.default($srcDOM);
						break;
				}
			} else {
			...
			(0, _jquery2.default)('div[data-effecttype]', _this.$wrapper).each(function (i, card) {
				var $card = (0, _jquery2.default)(card);
				var currentWidget;
				switch ($card.data('effecttype')) {
					case 'biquadfilter':
						currentWidget = new _biquad_filter2.default($card);
						break;
					case 'convolver':
						currentWidget = new _convolver2.default($card);
						break;
					case 'delay':
						currentWidget = new _delay2.default($card);
						break;
					case 'dynamicscompressor':
						currentWidget = new _dynamics_compressor2.default($card);
						break;
					case 'gain':
						currentWidget = new _gain2.default($card);
						break;
					case 'stereopanner':
						currentWidget = new _stereo_panner2.default($card);
						break;
					case 'waveshaper':
						currentWidget = new _wave_shaper2.default($card);
						break;
					case 'periodicwave':
						currentWidget = new _periodicwave2.default($card);
						break;
				}
				_this.widgetList.push(currentWidget);
				_this.initPromiseList.push(currentWidget.initPromise);
			});
			_jquery2.default.when.apply(_jquery2.default, _this.initPromiseList).done(function () {
				_jquery2.default.each(_this.widgetList, function (i, widget) {
					_this.nodeMap.push(widget.node);
				});
				for (var i = 0; i < _this.nodeMap.length - 1; i++) {
				...
				_this.nodeMap[_this.nodeMap.length - 1].connect(_audio_context2.default.destination);
			});
		}
	}]);
...
(function (global, factory) {
    ...
    function isWithin(element, selector) {
        element = $$1__default(element);
        return element.is(selector) || !!(isString(selector) ? element.parents(selector).length : $$1__default.contains(toNode(selector), element[0]));
    }
    ...
    function isInView(element, offsetTop, offsetLeft) {
        ...
        return rect.top >= -1 * offsetTop && rect.left >= -1 * offsetLeft && rect.top <= (window.innerHeight || document.documentElement.clientHeight) + offsetTop && rect.left <= (window.innerWidth || document.documentElement.clientWidth) + offsetLeft;
    }
    ...
    function toList(value) {
        return $$1.isArray(value) ? value : isString(value) ? value.split(',').map(function (value) {
            return value.trim();
        }) : [value];
    }
    ...
    // concat strategy
    strats.args = strats.attrs = strats.created = strats.events = strats.init = strats.ready = strats.connected = strats.disconnected = strats.destroy = function (parentVal, childVal) {

        parentVal = parentVal && !$$1.isArray(parentVal) ? [parentVal] : parentVal;

        return childVal ? parentVal ? parentVal.concat(childVal) : $$1.isArray(childVal) ? childVal : [childVal] : parentVal;
    };
    ...
    strats.defaults = strats.methods = function (parentVal, childVal) {
        return childVal ? parentVal ? $$1.extend(true, {}, parentVal, childVal) : childVal : parentVal;
    };
    ...
    */
    ...
    function internalAPI(UIkit) {
        ...
        UIkit.prototype._initEvents = function (unbind) {
            ...
            var events = this.$options.events,
                connect = function connect(event, key) {
    ...
    function componentAPI(UIkit) {
        ...
        UIkit.component = function (id, options) {
            ...
            UIkit[name] = function (element, data) {
                ...
                data = data || {};
                element = isString ? $$1__default(element)[0] : isJQuery(element) ? element[0] : element;
    ...
    var Toggable = {
        ...
        methods: {
            ...
            toggleElement: function toggleElement(targets, show, animate) {
                ...
                var queued = this.queued && !!this.animation[0],
                    toggles,
                    body = document.body,
            ...
            _toggleElement: function _toggleElement(el, show, animate) {
                ...
                var promise = (this.animation[0] === true && animate !== false ? this._toggleHeight : this.animation[0] && animate !== false ? this._toggleAnimation : this._toggleImmediate)(el, show);
            ...
            _toggleHeight: function _toggleHeight(el, show) {
                ...
                return Transition.cancel(el).then(function () {
                    ...
                    return show ? Transition.start(el, $$1.extend(this$1.initProps, { overflow: 'hidden', height: endHeight }), Math.round(this$1.duration * (1 - height / endHeight)), this$1.transition) : Transition.start(el, this$1.hideProps, Math.round(this$1.duration * (height / endHeight)), this$1.transition).then(function () {
        ...
        UIkit.component('cover', {
            ...
            update: {
                ...
                write: function write() {
                    ...
                    this.$el.css({ width: '', height: '' }).css(Dimensions.cover({ width: this.width || this.$el.width(), height: this.height || this.$el.height() }, { width: this.$el.parent().outerWidth(), height: this.$el.parent().outerHeight() }));
                },
            ...
            update: {
                ...
                read: function read() {
                    ...
                    var lastOffset = false,
                        elements = $$1__default(this.target, this.$el).css('minHeight', '');

                    this.rows = !this.row ? [this.match(elements)] : elements.toArray().reduce(function (rows, el) {
    ...
    function Icon(UIkit) {
        ...
        ['navbar-toggle-icon', 'overlay-icon', 'pagination-previous', 'pagination-next', 'totop'].forEach(function (name) {
                ...
                write: function write() {
                    ...
                    this.rows.forEach(function (row, i) {
                        return row.forEach(function (el, j) {
                            return $$1__default(el).toggleClass(this$1.margin, i !== 0).toggleClass(this$1.firstColumn, j === 0);
                        });
                    });
                },
        ...
        UIkit.component('scroll', {
            ...
            methods: {
                ...
                scrollToElement: function scrollToElement(el) {
                    ...
                    $$1__default('html,body').stop().animate({ scrollTop: parseInt(target, 10) || 1 }, this.duration, this.transition).promise().then(function () {
        ...
        UIkit.component('sticky', {
            ...
            update: [{
                ...
                write: function write() {
                    ...
                    var outerHeight = this.$el[0].offsetHeight,
                        el;
            ...
            methods: {
                ...
                hide: function hide() {
                    ...
                    this.$el.addClass(this.clsInactive).removeClass(this.clsFixed).removeClass(this.clsActive).css({ position: '', top: '', width: '' }).trigger('inactive');
                ...
                update: function update() {
                    ...
                    this.$el.css({
                        ...
                        top: top + "px",
                        width: this.width
                    }).addClass(this.clsFixed).toggleClass(this.clsActive, active).toggleClass(this.clsInactive, !active);
                }
        ...
        UIkit.component('toggle', {
            ...
            events: [{
                ...
                handler: function handler(e) {
                    ...
                    if (this.href || $$1__default(e.target).closest('a[href="#"], button').length || $$1__default(e.target).closest('a[href]') && (this.cls || !this.target.is(':visible'))) {
                        e.preventDefault();
                    }
...
var BiquadFilterWidget = function BiquadFilterWidget(wrapper) {
	_classCallCheck(this, BiquadFilterWidget);

	var _this = this;
	var initDtd = _jquery2.default.Deferred();
	_this.initPromise = initDtd.promise();
	_this.$wrapper = (0, _jquery2.default)(wrapper);
	_this.node = _audio_context2.default.createBiquadFilter();
	initDtd.resolve();
	_this.$wrapper.data({
		inited: true,
		widget: _this
...
exports.default = BiquadFilterWidget;
...
var ConvolverWidget = function ConvolverWidget(wrapper) {
	_classCallCheck(this, ConvolverWidget);

	var _this = this;
	var initDtd = _jquery2.default.Deferred();
	_this.initPromise = initDtd.promise();
	_this.$wrapper = (0, _jquery2.default)(wrapper);
	_this.node = _audio_context2.default.createConvolver();
	initDtd.resolve();
	_this.$wrapper.data({
		inited: true,
		widget: _this
...
exports.default = ConvolverWidget;
...
var DelayWidget = function () {
	function DelayWidget(wrapper) {
		_classCallCheck(this, DelayWidget);

		var _this = this;
		_this.initDtd = _jquery2.default.Deferred();
		...
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('.input-form', _this.$wrapper).on('submit', function (e) {
			e.preventDefault();
			_this.setParam({
				delay: (0, _jquery2.default)('.delay-input', _this.$wrapper).val()
			});
			(0, _jquery2.default)('button', _this.$wrapper).hide();
			return false;
		});
		_this.$wrapper.data({
			inited: true,
	...
	_createClass(DelayWidget, [{
		...
		value: function setParam(option) {
			...
			if (option) {
				...
				_this.node = _audio_context2.default.createDelay(delay);
...
var DynamicsCompressorWidget = function () {
	function DynamicsCompressorWidget(wrapper) {
		_classCallCheck(this, DynamicsCompressorWidget);

		var _this = this;
		_this.initDtd = _jquery2.default.Deferred();
		_this.initPromise = _this.initDtd.promise();
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		_this.node = _audio_context2.default.createDynamicsCompressor();
		var tmpl = '';
		tmpl += '<div>' + '<p>threshold(db):<span class="threshold-val">-50</span></p>' + '<p><input type="range" min="-100" max="0" value="-50" class="threshold-input" /></p>' + '<p>knee(db):<span class="knee-val">40</span></p>' + '<p><input type="range" min="0" max="40" value="40" class="knee-input" /></p>' + '<p>ratio(db):<span class="ratio-val">12</span></p>' + '<p><input type="range" min="1" max="20" value="12" class="ratio-input" /></p>' +
		// '<p>reduction(float):</p>'+
		// '<p><input type="text" value="-20" class="reduction-input" /></p>'+
		'<p>attack(second):<span class="range-val">0</span></p>' + '<p><input type="range" min="0" max="1" value="0" step="0.01" class="attack-input" /></p>' + '<p>release(second):<span class="release-val">0.25</span></p>' + '<p><input type="range" min="0" max="1" value="0.25" step="0.01" class="release-input" /></p>' + '</div>';
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('input', _this.$wrapper).on('change', function (e) {
			e.preventDefault();
			_this.setParam({
				threshold: (0, _jquery2.default)('.threshold-input', _this.$wrapper).val(),
				knee: (0, _jquery2.default)('.knee-input', _this.$wrapper).val(),
				ratio: (0, _jquery2.default)('.ratio-input', _this.$wrapper).val(),
				reduction: (0, _jquery2.default)('.reduction-input', _this.$wrapper).val(),
				attack: (0, _jquery2.default)('.attack-input', _this.$wrapper).val(),
				release: (0, _jquery2.default)('.release-input', _this.$wrapper).val()
			});
			return false;
		});
		_this.$wrapper.data({
	...
	_createClass(DynamicsCompressorWidget, [{
		key: 'setParam',
		value: function setParam(option) {
			var _this = this;
			if (option) {
				...
				(0, _jquery2.default)('.threshold-val', _this.$wrapper).text(_this.node.threshold.value);
				(0, _jquery2.default)('.knee-val', _this.$wrapper).text(_this.node.knee.value);
				(0, _jquery2.default)('.ratio-val', _this.$wrapper).text(_this.node.ratio.value);
				(0, _jquery2.default)('.reduction-val', _this.$wrapper).text(_this.node.reduction.value);
				(0, _jquery2.default)('.attack-val', _this.$wrapper).text(_this.node.attack.value);
				(0, _jquery2.default)('.release-val', _this.$wrapper).text(_this.node.release.value);
			}
		}
	}]);

	return DynamicsCompressorWidget;
}();

exports.default = DynamicsCompressorWidget;
...
var FileAudioWidget = function () {
	function FileAudioWidget(wrapper) {
		_classCallCheck(this, FileAudioWidget);
		...
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('.file-form', _this.$wrapper).on('submit', function (e) {
			e.preventDefault();
			_this.setParam({
				fileUrl: (0, _jquery2.default)('.file-input', _this.$wrapper).val()
			});
			(0, _jquery2.default)('.file-form button', _this.$wrapper).hide();
			return false;
		});
		_this.$wrapper.data({
			inited: true,
		...
		value: function setParam(option) {
			...
			if (option) {
				...
				request.onload = function () {
					_audio_context2.default.decodeAudioData(request.response, function (buffer) {
						_this.node = _audio_context2.default.createBufferSource();
				...
				request.send();
				(0, _jquery2.default)('.file-input', _this.$wrapper).val(option.fileUrl);
			}
		}
	}]);

	return FileAudioWidget;
}();

exports.default = FileAudioWidget;
...
var GainWidget = function () {
	function GainWidget(wrapper) {
		_classCallCheck(this, GainWidget);

		var _this = this;
		_this.initDtd = _jquery2.default.Deferred();
		_this.initPromise = _this.initDtd.promise();
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		_this.node = _audio_context2.default.createGain();
		var tmpl = '';
		tmpl += '<div>' + '<p>gain:<span class="gain-val">1</span></p>' + '<p><input type="range" min="0" max="2" step="0.1" value="1" class="gain-input" /></p>' + '</div>';
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('input', _this.$wrapper).on('change', function (e) {
			e.preventDefault();
			_this.setParam({
				gain: (0, _jquery2.default)('.gain-input', _this.$wrapper).val()
			});
			return false;
		});
		_this.$wrapper.data({
	...
	_createClass(GainWidget, [{
		key: 'setParam',
		value: function setParam(option) {
			var _this = this;
			if (option) {
				_this.node.gain.value = 'undefined' == typeof option.gain ? 1 : +option.gain;
				(0, _jquery2.default)('.gain-val', _this.$wrapper).text(_this.node.gain.value);
			}
		}
	}]);
...
var MicWidget = function () {
	function MicWidget(wrapper) {
		_classCallCheck(this, MicWidget);
		...
		navigator.getUserMedia({
			...
			_this.node = _audio_context2.default.createMediaStreamSource(stream);
			initDtd.resolve();
		}, function (err) {
			console.warn(err);
		});
...
var OscillatorWidget = function () {
	function OscillatorWidget(wrapper) {
		_classCallCheck(this, OscillatorWidget);
		...
		tmpl += '</select></p></div>';
		(0, _jquery2.default)('<div>' + tmpl + '</div>').appendTo(_this.$wrapper);
		var $fInput = (0, _jquery2.default)('.f-input', _this.$wrapper);
		var $dtInput = (0, _jquery2.default)('.dt-input', _this.$wrapper);
		var $typeInput = (0, _jquery2.default)('.type-input', _this.$wrapper);
		...
		_this.node = _audio_context2.default.createOscillator();
		_this.setParam();
		_this.node.start(0);
		initDtd.resolve();
		_this.$wrapper.data({
	...
	_createClass(OscillatorWidget, [{
		...
		value: function setParam(option) {
			...
			(0, _jquery2.default)('.f-num', _this.$wrapper).text(this.node.frequency.value);
			(0, _jquery2.default)('.dt-num', _this.$wrapper).text(this.node.detune.value);
			(0, _jquery2.default)('.f-input', _this.$wrapper).val(this.node.frequency.value);
			(0, _jquery2.default)('.dt-input', _this.$wrapper).val(this.node.detune.value);
		}
	}]);

	return OscillatorWidget;
}();

exports.default = OscillatorWidget;
...
var PeriodicWaveWidget = function () {
	function PeriodicWaveWidget(wrapper) {
		_classCallCheck(this, PeriodicWaveWidget);
		...
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		var tmpl = '';
		tmpl += '<div>' + '<form action="#" class="param-form">' + '<p>real:<span class="real-val">0,1</span></p>' + '<p><input type="text" value="0,1" class="real-input" /></p>' + '<p>imag:<span class="imag-val">0,0</span></p>' + '<p><input type="text" value="0,0" class="imag-input" /></p>' + '<p>disableNormalization: <input type="checkbox" class="disablenormalization-input" /></p>' + '<p><button type="submit">set</button></p>' + '</form>' + '</div>';
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('.param-form', _this.$wrapper).on('submit', function (e) {
			e.preventDefault();
			_this.setParam({
				real: (0, _jquery2.default)('.real-input', _this.$wrapper).val(),
				imag: (0, _jquery2.default)('.imag-input', _this.$wrapper).val(),
				disableNormalization: (0, _jquery2.default)('.disablenormalization-input', _this.$wrapper).prop('checked')
			});
			(0, _jquery2.default)('button', _this.$wrapper).hide();
			return false;
		});
	...
	_createClass(PeriodicWaveWidget, [{
		key: 'setParam',
		value: function setParam(option) {
			var _this = this;
			if (option) {
				...
				imag[0] = imagVal[0];
				imag[1] = imagVal[1];
				_this.node = _audio_context2.default.createPeriodicWave(real, imag, {
	...
	return PeriodicWaveWidget;
}();

exports.default = PeriodicWaveWidget;
...
var StereoPannerWidget = function () {
	function StereoPannerWidget(wrapper) {
		_classCallCheck(this, StereoPannerWidget);

		var _this = this;
		_this.initDtd = _jquery2.default.Deferred();
		_this.initPromise = _this.initDtd.promise();
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		_this.node = _audio_context2.default.createStereoPanner();
		var tmpl = '';
		tmpl += '<div>' + '<p>panner:<span class="pan-val">1</span></p>' + '<p><input type="range" min="-1" max="1" step="0.1" value="1" class="pan-input" /></p>' + '</div>';
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('input', _this.$wrapper).on('change', function (e) {
			e.preventDefault();
			_this.setParam({
				pan: (0, _jquery2.default)('.pan-input', _this.$wrapper).val()
			});
			return false;
		});
		_this.$wrapper.data({
	...
	_createClass(StereoPannerWidget, [{
		key: 'setParam',
		value: function setParam(option) {
			var _this = this;
			if (option) {
				_this.node.pan.value = 'undefined' == typeof option.pan ? 0 : +option.pan;
				(0, _jquery2.default)('.pan-val', _this.$wrapper).text(_this.node.pan.value);
			}
		}
	}]);

	return StereoPannerWidget;
}();

exports.default = StereoPannerWidget;
...
var WaveShaperWidget = function () {
	function WaveShaperWidget(wrapper) {
		_classCallCheck(this, WaveShaperWidget);

		var _this = this;
		_this.initDtd = _jquery2.default.Deferred();
		_this.initPromise = _this.initDtd.promise();
		_this.$wrapper = (0, _jquery2.default)(wrapper);
		_this.node = _audio_context2.default.createWaveShaper();
		_this.node.oversample = '4x';
		var tmpl = '';
		tmpl += '<div>' + '<p>amount:<span class="amount-val">50</span></p>' + '<p><input type="range" min="1" max="400" value="50" class="amount-input" /></p>' + '</div>';
		(0, _jquery2.default)(tmpl).appendTo(_this.$wrapper);
		(0, _jquery2.default)('input', _this.$wrapper).on('change', function (e) {
			e.preventDefault();
			_this.setParam({
				amount: (0, _jquery2.default)('.amount-input', _this.$wrapper).val()
			});
			return false;
		}).change();
		_this.$wrapper.data({
	...
	_createClass(WaveShaperWidget, [{
		key: 'setParam',
		value: function setParam(option) {
			var _this = this;
			if (option) {
				_this.node.curve = 'undefined' == typeof option.amount ? makeDistortionCurve() : makeDistortionCurve(option.amount);
				(0, _jquery2.default)('.amount-val', _this.$wrapper).text(option.amount);
			}
		}
	}]);

	return WaveShaperWidget;
}();

exports.default = WaveShaperWidget;
...
var _audio_line = __webpack_require__(2);

var _audio_line2 = _interopRequireDefault(_audio_line);
...
function initAudioLine($line) {
	...
	_uikit2.default.grid((0, _jquery2.default)('.effect-wrapper', $line));
}

function dropItem($drag, $target) {
	$target.removeClass('uk-text-center hover-zone').addClass('has-content').find('.placeholder').remove();

	$drag.attr('style', '').appendTo($target);
	...
	switch ($drag.data('type')) {
		case 'src':
			(0, _jquery2.default)('.src-wrapper', $target).empty().append($drag);
			break;
		case 'effect':
			var $li = (0, _jquery2.default)('<li></li>');
			$li.append($drag).appendTo((0, _jquery2.default)('.effect-wrapper', $target));
			_uikit2.default.grid((0, _jquery2.default)('.effect-wrapper', $target));
			break;
		case 'analyser':
			(0, _jquery2.default)('.analyser-wrapper', $target).empty().append($drag);
			break;
	}
	if (!$target.data('audio')) {
		var lineAudio = new _audio_line2.default($target);
...
(0, _jquery2.default)(function () {
	...
	var dragOffset = {};
	(0, _jquery2.default)('body').delegate('.dragable', 'mousedown', function (e) {
		var $oItem = (0, _jquery2.default)(this);
		$draggingItem = (0, _jquery2.default)(this).clone().removeClass('dragable').appendTo('body').css({
			position: 'fixed',
		...
		if ($draggingItem) {
			...
			var $tempTarget = null;
			(0, _jquery2.default)('.dropable').each(function (i, zone) {
				var $zone = (0, _jquery2.default)(zone);
				if (e.pageX >= $zone.offset().left && e.pageX <= $zone.offset().left + $zone.outerWidth() && e.pageY >= $zone.offset().top && e.pageY <= $zone.offset().top + $zone.outerHeight()) {
